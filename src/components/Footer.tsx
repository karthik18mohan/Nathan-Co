...
export function Footer() {
  ...
  return (
    <footer className="border-t border-stone bg-paper py-12">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-3">
          <p className="text-sm font-medium uppercase tracking-[0.35em] text-oxford">
            Nathan &amp; Co.
          </p>
          <p className="text-sm text-charcoal/70">
            © {year} Nathan &amp; Co. All rights reserved.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 text-charcoal/70">
          {socials.map((social) => (
            <a
              key={social.label}
              href={social.href}
              className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-stone transition hover:border-oxford/40 hover:text-oxford"
